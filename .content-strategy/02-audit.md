...
2. **No download/install path** — The only way to use Anvil is cloning and building from source (Rust + Node.js). No pre-built binaries, no GitHub Releases link. Eliminates 90%+ of potential users.
3. **Getting Started buried at line 190** — A developer wanting to try Anvil must scroll past Features, Keyboard Shortcuts, and Tech Stack (60% of the README).
4. **Placeholder clone URL** — `git clone https://github.com/user/anvil.git` is broken. The single most action-oriented line fails.
...
13. **Inconsistent section density** — "What is Anvil?" is 8 lines; "Features" is 58 lines; "Keyboard Shortcuts" is 43 lines.
...
### Critical
14. **H1 title links to wrong repo** — `<a href="https://github.com/acaprino/anvil-toolset">Anvil</a>` links to a different repository. Trust-breaking bug.
15. **Placeholder clone URL** — `github.com/user/anvil.git` will 404 for anyone copy-pasting.
...
17. **"Why Anvil?" fails to address the core objection** — "I already run `claude` in my terminal, why do I need a separate app?" The section lists attributes without contrasting against the status quo.
18. **"What is Anvil?" uses jargon** — "project-aware terminal multiplexer" undersells the product. Not every Claude Code user knows what a terminal multiplexer is.
...
21. **"Pick a project. Pick a model. Hit Enter. Code." is buried** — The best copy in the README is hidden in the "What is Anvil?" section instead of being the hero subtitle.
...
8. **"Why Anvil?" bullet formatting** — Good benefit framing with em-dash explanations.
