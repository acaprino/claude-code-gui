...
echo ============================================
echo   Anvil - Tauri App Build (MSVC link.exe)
echo ============================================
echo.
...
:: Remove .cargo/config.toml linker override so MSVC link.exe is used
set "CARGO_CONFIG=%~dp0app\src-tauri\.cargo\config.toml"
if exist "%CARGO_CONFIG%" (
    echo Removing custom linker config to use MSVC link.exe...
    del "%CARGO_CONFIG%"
)
