...
function AppContent() {
  ...
  // Load bundled marketplace plugin paths (Claude Code GUI-exclusive by default)
  ...
  useEffect(() => {
    if (activeTab.type === "agent" && activeTab.projectName) {
      ...
      appWindow.setTitle(`Claude Code GUI \u2014 ${activeTab.projectName}${suffix}`);
    } else {
      appWindow.setTitle("Claude Code GUI");
    }
  ...
  const handleConfigChange = useCallback((tabId: string, update: { modelIdx?: number; effortIdx?: number; permModeIdx?: number }) => {
    ...
    // Model and perm mode changes are synced to sidecar live. Effort takes effect on next session.
